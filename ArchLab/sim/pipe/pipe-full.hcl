# 蒋衍 1600013239 出品
#######################################################################
#           Begin of my description in pipe-full.hcl                  #
#######################################################################
# For IIADDL instruction:
#  Same as IIADDL in seq-full.hcl
# For ILEAVE instruction:
#  Almost the same as ILEAVE in seq-full.hcl, except in pipeline 
#  register control section. ILEAVE reads from memory to update R[%ebp]
#  will cause a load-use hazard. Since ILEAVE is equalvalent instruction 
#  of IRRMOVE %ebp, %esp followed by IPOPQ %rsp, just following 
#  operations of IPOPQ int pipeline register control section solve this
#  problem.
#######################################################################
#            End of my description in pipe-full.hcl                   #
#######################################################################
...
bool instr_valid = f_icode in 
	{ INOP, IHALT, IRRMOVL, IIRMOVL, IRMMOVL, IMRMOVL,
	  IOPL, IJXX, ICALL, IRET, IPUSHL, IPOPL, IIADDL, ILEAVE };  # altered for IIADDL
...
bool need_regids =
	f_icode in { IRRMOVL, IOPL, IPUSHL, IPOPL, 
		     IIRMOVL, IRMMOVL, IMRMOVL, IIADDL };   # altered for IIADDL

# Does fetched instruction require a constant word?
bool need_valC =
	f_icode in { IIRMOVL, IRMMOVL, IMRMOVL, IJXX, ICALL, IIADDL };   # altered for IIADDL
...
int d_srcA = [
	...
	D_icode in { ILEAVE } : REBP;        # add a line for instrction ILEAVE
	1 : RNONE; # Don't need register
];

## What register should be used as the B source?
int d_srcB = [
	D_icode in { IOPL, IRMMOVL, IMRMOVL, IIADDL  } : D_rB;    # altered for IIADDL
...
int d_dstE = [
	D_icode in { IRRMOVL, IIRMOVL, IOPL, IIADDL} : D_rB;     # altered for IIADDL
	D_icode in { IPUSHL, IPOPL, ICALL, IRET } : RESP;
	D_icode in { ILEAVE } : RESP;        # add a line for instrction ILEAVE
...
int aluA = [
	...
	E_icode in { IIRMOVL, IRMMOVL, IMRMOVL, IIADDL } : E_valC;     # altered for IIADDL
...
bool set_cc = E_icode in {IOPL, IIADDL} &&            # altered for IIADDL
