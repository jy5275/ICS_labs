...
int f_pc = [
	...
	# Untaken forward jump. Should jump to valC, but didn't. Jump to previous valC (M_valE) now!
	M_icode == IJXX && M_ifun && M_valE > M_valA && M_Cnd : M_valE;  
	# Mistaken backward jump. Should remain at valP but jumpped. Jump to previous valP (M_valA) now!
	M_icode == IJXX && M_ifun && M_valE <= M_valA && !M_Cnd : M_valA;
...
int f_predPC = [
	...
	f_icode in { IJXX, ICALL } && f_ifun != 0: f_valC;  # same as pipe-full.hcl
	f_icode == IJXX && f_valP > f_valC: f_valC     # Jump backward, positive prediction
	f_icode == IJXX: f_valP                        # Jump forward, negative prediction  
	1 : f_valP;
];
...
int aluA = [
	...
	E_icode == IJXX && E_ifun : E_valC;      # valE assists to transfer valC
...
int aluB = [
	...
	E_icode == IJXX && E_ifun : 0;           # valE assists to transfer valC
	# Other instructions don't need ALU
];
...
int alufun = [
	E_icode == IOPL : E_ifun;
	1 : ALUADD;                              # IJXX fall in this line
];
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && E_ifun && E_valC < E_valA && !e_Cnd) || (E_icode == IJXX && E_ifun && E_valC >= E_valA && e_Cnd)
