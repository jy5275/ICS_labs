 ...
 * 蒋衍 1600013239 出品 
 * Four built-in command: jobs, fg, bg, quit.
 * argv[0] apart from built-in commands are treated as pathname of executable
 * file. However, it beheaves differently from linux shell in some cases, 
 * which includes but not limited as below:
 *  - Process turned foreground by fg command could not read from terminal. 
 *  - Jobs' state could change to ST ONLY in sigchld_handler, which means that 
 *    jobs' state maintained in the joblist may be not correct.
 * Fortunately, this is just a lab work.
 * Enjoy it.
 */
#include <assert.h>
#include <stdio.h>
...
int main (int argc, char **argv) {
    ...
    while ((c = getopt(argc, argv, "hvp")) != EOF) {  
        ...
        case 'h': 
        	usage(); break;           /* help */
        case 'v': 
        	verbose = 1; break;       /* emit additional diagnostic info */
        case 'p': 
        	emit_prompt = 0; break;   /* don't print a prompt */
        default: usage();
        }
    }
...
void eval(char *cmdline) {
    ...
    int fd_in, fd_out;				/* discriptive number for redirection */
    
    int _stdout, _stdin, _stderr;   /* initial STDIN, STDOUT, STDERR 
                                     * for re-redirection 
                                     */
	struct job_t *pjob = NULL;      /* BG and FG cmd pointer */
    struct cmdline_tokens tok;
    sigset_t mask_3, prev, mask_all;
	pid_t pid;
	...
	/* Main process shouldn't be interrupted while waiting for FG 
	 * or before addjob is done, so block all interuptive signals
	 * in trace files.
	 */
	sigfillset(&mask_all);
	sigemptyset(&mask_3); 
	...
	/* Open files and I/O redirection */
	if (tok.infile) {
		if ((_stdin = dup(STDIN_FILENO)) < 0) 
	...
	if (tok.outfile) {
		if ((_stdout = dup(STDOUT_FILENO)) < 0) 
			unix_error("dup error");
		if ((_stderr = dup(STDERR_FILENO)) < 0)
			unix_error("dup error");
	...
	if (tok.builtins == BUILTIN_NONE) {
		...
		/* ================ Begin of child-exclusive code============== */
			...
			/* Cannot find pathname argv[0], terminate */
		...
		/* Add to joblist and wait for FG job to terminate */
		if (!bg) {
			addjob(job_list, pid, FG, cmdline);
			struct job_t *pjob = getjobpid(job_list, pid);
			while (pjob->jid && pjob->state != ST) 
				sigsuspend(&prev); 
		}
		/* Add BG job to joblist */
		else {
			addjob(job_list, pid, BG, cmdline);
			pjob = getjobpid(job_list, pid);
			printf("[%d] (%d) %s\n", pjob->jid, pid, cmdline);
		}
		...
		/* Extract pjob from argv */
		/* By jid */
		if (tok.argv[1][0] == '%') {
			bg_jid = atoi(&tok.argv[1][1]);
			pjob = getjobjid(job_list, bg_jid);
			/* If such jid do not exit...return */
			if (pjob == NULL) {
		...
		/* By pid */
			...
			pjob = getjobpid(job_list, bg_pid);
			/* If such pid do not exit...return  */
			if (pjob == NULL) {
		...
		/* Send SIGCONT and change job state to BG */
		if (kill(-pjob->pid, SIGCONT) < 0)
		...
		/* Block SIGCHLD to avoid indefinite suspend */
		...
		if (tok.argv[1][0] == '%') {
			fg_jid = atoi(&tok.argv[1][1]);
			pjob = getjobjid(job_list, fg_jid);		
			/* If such jid do not exit...return */
			if (pjob == NULL) {
				printf("%%%d: No such job\n", fg_jid);
				return;
			...
			pjob = getjobpid(job_list, fg_pid);
			/* If such pid do not exit...return  */
			if (pjob == NULL) {
				printf("(%d): No such process\n", fg_pid);
				return;
		...
		if (kill(-pjob->pid, SIGCONT) < 0)
			unix_error("kill error");
		/* Waiting for FG job to terminate */
		while (pjob->jid && pjob->state != ST)  
			/* Stay in loop while the FG job still exists. If interrupted
			 * by a SIGCHLD from another process, that process could also 
			 * be reaped as usual. Only the kernal has reaped all zombies
			 * can we return from sigchld_handler, (then sigsuspend) to
			 * check wheather the loop condition still satisfy. 
			 */
			sigsuspend(&prev);
		...
		/* The only normal exit of tsh's routine */
		exit(0);
	
	/* Close files and I/O re-redirection */
	if (tok.infile) {
	...
	if (tok.outfile) {
		if (dup2(_stdout, STDOUT_FILENO) < 0)
			unix_error("dup2 error");
		if (dup2(_stderr, STDERR_FILENO) < 0)
			unix_error("dup2 error");
		if (close(fd_out) < 0)
			unix_error("close error");
	}
...
int parseline(const char *cmdline, struct cmdline_tokens *ptok) {
    ...
    while (buf < endbuf) {
        ...
        /* Returned by strchr(); means closing quote not found. */
        if (next == NULL) { 
...
void sigint_handler(int sig) {	
	...
	/* Send SIGINT to each process in FG group */
	if (fg_pid) 
