 ...
 * mm-seg7.c	5 + 6 (88 = 49 + 39, extremity?) [22:52, Sun]
...
static int exhaustcheck = 1;	// Optionally print info of each block by sequence
...
int mm_init (void) {
    ...
    /* next-ptr on each block ptr to pTail in the beginning */
    int i = 0;
    while (i < 10) {
    	_GET(2*WSIZE*i + WSIZE) = _pTail;
    	i++;
    }
    heap_listp = pTail;
    _heap_listp = _pTail;
    _GET(_pTail + WSIZE) = 0;	/*Prologue next-ptr*/
...
static unsigned _InsertBlock (unsigned _p, size_t index) {	/* insert new block into beg list[index] */
	unsigned _pbase = _heap + index*DSIZE;
	unsigned _it = _pbase;
	size_t len = _GET_SIZE(_HDRP(_p));
	while (_GET(_it + WSIZE) != _pTail && _GET_SIZE(_HDRP(_GET(_it + WSIZE))) < len) {	// Break if [it ptr to end node] || [it ptr to node's next longer than p]
		_it = _GET(_it + WSIZE);		// Go to next free block
	} 
...
void mm_checkheap(int lineno) {
	...
	for (i = 0; i < 10; i++) {
		...
		_it = _heap + DSIZE*i;
		while (_GET(_it + WSIZE) != 0) {	// Break if _it ptr to pTail
